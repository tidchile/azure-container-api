...
var ExponentialRetryPolicyFilter = azrStorage.ExponentialRetryPolicyFilter;
/*
...
var copyBlobs = function(sourceAccountName, sourceAccountKey, sourceContainer, sourceBlob, destAccountName, destAccountKey, destContainer, destBlob){
    ...
    blobSvcSource = blobSvcSource.withFilter(new ExponentialRetryPolicyFilter());
    ...
    return new Promise(function (resolve, reject) {
        blobSvcSource.getContainerAcl(sourceContainer, function(err, permission){
            if (err) {
                ...
                blobSvcDest = blobSvcDest.withFilter(new ExponentialRetryPolicyFilter());
...
var copyBlobsByPrefix = function(sourceAccountName, sourceAccountKey, sourceContainer, sourcePrefix, destAccountName, destAccountKey, destContainer, destPrefix){
    ...
    if (sourcePrefix === null || sourcePrefix === undefined) {
        throw new Error('sourcePrefix cannot be null.');
    }
    if (destAccountName === null || destAccountName === undefined) {
        throw new Error('destAccountName cannot be null.');
    }
    ...
    var endpointSuffix = '.blob.core.windows.net';
    var protocol = 'https://';
    var sourceBlobEndpoint =  protocol + sourceAccountName + endpointSuffix + '/' + sourceContainer ;
    var destBlobEndpoint =   protocol  + destAccountName +  endpointSuffix;
    var blobSvcSource = createBlobService(sourceAccountName, sourceAccountKey, sourceBlobEndpoint);
    blobSvcSource = blobSvcSource.withFilter(new ExponentialRetryPolicyFilter());
    var sasToken;
    ...
    return new Promise(function (resolve,reject) {
        ...
        blobSvcSource.getContainerAcl(sourceContainer, function(err, permission){
            if (err) {
                ...
                getBlobsContainer(sourceAccountName, sourceAccountKey, sourceContainer, sourcePrefix)
                .then(function(listBlobs){
                     listBlobs = listBlobs.blobs;
                     var blobSvcDest = createBlobService(destAccountName, destAccountKey, destBlobEndpoint);
                     blobSvcDest = blobSvcDest.withFilter(new ExponentialRetryPolicyFilter());
                     return Promise.all(
                        listBlobs.map(function(blob){
                            if (sharedAccessPolicy)
                                sasToken = blobSvcSource.generateSharedAccessSignature(sourceContainer,
                                                                           blob.name, sharedAccessPolicy);
                            var sourceUri = sourceBlobEndpoint + '/' + blob.name;
                            if(sasToken)
                                sourceUri += '?' + sasToken;
                            var destBlob = destPrefix ? blob.name.replace(sourcePrefix, destPrefix):blob.name;
...
module.exports = {
    ...
    ,getSize: getSize
    ,copyBlobs: copyBlobs
    ,copyBlobsByPrefix: copyBlobsByPrefix
