...
var deleteCluster = function(credential, clusterName, location){
    ...
    return new Promise(function(resolve, reject) {
        ...
        getClusterList(credential).then(function(list) {
            ...
            for (var i = 0; i < list.resources.length; i++){
                if (list.resources[i].name==clusterName) {
                    cluster = list.resources[i];
            ...
            if (cluster){
                var tokens = cluster.type.split('/');
                var namespace = tokens[0];

                cluster = Object.assign(cluster ,azrUtil.getResourceInformation(cluster.id));

                cluster.resourceProviderNamespace = tokens[0];
                cluster.resourceType = tokens[1];
                tokens[1]= tokens[1].toLowerCase();
                azrResources.getProvider(credential, namespace).then(function(providerResult){
                    var resourcesTypesArr = providerResult.provider.resourceTypes;
                    for (var i = 0; i < resourcesTypesArr.length; i++){
                        if (resourcesTypesArr[i].name.toLowerCase()==tokens[1]) {
                            cluster.resourceProviderApiVersion = resourcesTypesArr[i].apiVersions;
                            break;
                        }
                    }
                    console.log(cluster);
                    //var regionCloudServiceName = azureUtil.getNameSpace(credential.subscriptionId, 'hdinsight', cluster.location);
                    var mgmClt = azureHDInsight.createHDInsightCluster2ManagementClient(cluster.resourceGroup, credential);
                ...
                reject(new Error("The resource with name "+ clusterName +" was not found."));
        }).catch(function(err){
            console.error(err.stack);
            reject(err);
        });
