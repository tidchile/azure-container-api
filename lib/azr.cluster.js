...
var deleteCluster = function(credential, clusterName, location) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        getClusterList(credential).then(function(list) {
            ...
            if (cluster) {
                var resourceType = cluster.type    
                    ,j = 1 + resourceType.lastIndexOf('/'); 
                i = resourceType.indexOf('/')
                
                cluster = Object.assign(cluster, resourceUtil.getResourceInformation(cluster.id));
                cluster.resourceProviderNamespace = resourceType.substring(0, i);
                cluster.resourceType = resourceType.substring(j);
                cluster.parentResourcePath = cluster.parentResource;
                
                var mgmClt = azureHDInsight.createHDInsightCluster2ManagementClient(cluster.resourceGroup, credential);
                    mgmClt.clusterManagement.deleteMethod(clusterName,
                        function(err, result) {
                            if (err) {
                                reject(err);
                            } else {
